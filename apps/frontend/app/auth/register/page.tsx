...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setIsLoading(true)
    try { await authApi.register({ email, password, fullName }); router.push('/auth/login') }
    ...
    <main className="flex min-h-screen items-center justify-center bg-zinc-50 px-4">
      <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <div className="mb-6 text-center sm:mb-8">
          ...
          <p className="mt-1.5 text-sm text-slate-500">Comienza a gestionar tu tienda</p>
        </div>

        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-slate-700">Nombre completo</label>
            <input type="text" placeholder="Tu nombre" value={fullName} onChange={(e) => setFullName(e.target.value)} required
        ...
        <p className="mt-6 text-center text-sm text-slate-500">
          ¿Ya tienes cuenta?{' '}
          <Link href="/auth/login" className="font-semibold text-brand-600 hover:text-brand-700 transition-colors">Inicia sesión</Link>
        </p>
      </div>
    </main>
  )
