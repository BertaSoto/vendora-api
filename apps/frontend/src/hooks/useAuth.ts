  ...
  const login = useCallback(async ({ email, password }: LoginCredentials): Promise<boolean> => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const message = typeof errData.error === 'string' ? errData.error : 'Credenciales incorrectas'
      ...
      document.cookie = `vendora_token=${token}; path=/; max-age=${60 * 60 * 24 * 7}; SameSite=Strict`
      setState({ token, isAuthenticated: true, isLoading: false, error: null })
      return true
    } catch (err) {
