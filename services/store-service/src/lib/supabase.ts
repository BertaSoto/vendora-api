...
function getClient(): SupabaseClient {
  ...
  if (!process.env.SUPABASE_URL) {
    console.error('[store-service] SUPABASE_URL is not configured')
    throw new Error('SUPABASE_URL is required. Asegurate de configurarlo en tu .env.')
  }
  if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
    console.error('[store-service] SUPABASE_SERVICE_ROLE_KEY is not configured')
  ...
  try {
    ...
    console.error('[store-service] SUPABASE_URL is invalid (check format: https://<ref>.supabase.co)')
  }
